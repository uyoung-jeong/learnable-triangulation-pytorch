...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default=path.join('data', 'model_m.pkl'))
...
class SMPLModel(Module):
  def __init__(self, device=None, model_path='./model.pkl'):
    ...
    """
    print(f'self.J_regressor.shape:{self.J_regressor.shape}') # [24, 6890]
    print(f'self.weights.shape:{self.weights.shape}') # [6890, 24]
    print(f'self.posedirs.shape:{self.posedirs.shape}') # [6890, 3, 207]
    print(f'self.v_template.shape:{self.v_template.shape}') # [6890, 3]
    print(f'self.shapedirs.shape:{self.shapedirs.shape}') # [6890, 3, 10]
    print(f'self.kintree_table.shape:{self.kintree_table.shape}') # (2, 24)
    print(f'self.faces.shape:{self.faces.shape}') # (13776, 3)
    """
  ...
  def forward(self, betas, pose, trans, simplify=False):
    ...
    """
    print(f'v_shaped.shape:{v_shaped.shape}') # [6890, 3]
    print(f'J.shape:{J.shape}') # [24, 3]
    print(f'R_cube_big.shape:{R_cube_big.shape}') # [24, 3, 3]
    """
    ...
    """
    print(f'T.shape:{T.shape}') # [6890, 4, 4]
    print(f'rest_shape_h.shape:{rest_shape_h.shape}') # [6890, 4]
    print(f'trans.shape:{trans.shape}') # [3]
    print(f'v.shape:{v.shape}') # [6890, 3]
    print(f'result.shape:{result.shape}') # [6890, 3]
    """
...
def test_gpu(gpu_id=[0]):
  ...
  model = SMPLModel(device=device, model_path = args.model_path)
