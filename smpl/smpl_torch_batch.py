...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default=path.join('data', 'smpl', 'model_m.pkl'))
...
class SMPLModel(Module):
  ...
  def forward(self, betas, pose, trans, simplify=False):
    ...
    #print(f'result.shape:{result.shape}') # [32, 6890, 3]
    #print(f'self.joint_regressor.shape:{self.joint_regressor.shape}') # [24, 6890]
...
def test_gpu(gpu_id=[0]):
  ...
  for i in range(result.shape[0]):
      print(f'saving obj {outmesh_path.format(i)}')
