...
def prepare_smpl_batch(batch, device, config, is_train=True):
    ...
    # smpl keypoints
    smpl_keypoints_3d_batch_gt = torch.from_numpy(np.stack(batch['smpl_keypoints_3d'], axis=0)[:, :, :]).float().to(device)
    ...
    return images_batch, keypoints_3d_batch_gt, smpl_keypoints_3d_batch_gt, keypoints_3d_validity_batch_gt, proj_matricies_batch
