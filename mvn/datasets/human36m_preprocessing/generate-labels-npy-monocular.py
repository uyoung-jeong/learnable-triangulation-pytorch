"""
    Generate 'labels.npy' for monocular 'human36m.py'
    Usage: `python3 generate-labels-npy-monocular.py <path/to/Human3.6M-root> <path/to/una-dinosauria-data/h36m> <path/to/bboxes-Human36M-squared.npy>`
"""
import os, sys
import numpy as np
import h5py
...
destination_file_path = os.path.join(h36m_root, "extra", f"human36m-monocular-labels-{BBOXES_SOURCE}bboxes.npy")
...
pbar = tqdm(total=len(retval['subject_names']), desc='filling table')
for subject_idx, subject in enumerate(retval['subject_names']):
    ...
    for action_idx, action in enumerate(retval['action_names']):
        ...
        for camera_idx, camera in enumerate(retval['camera_names']):
            ...
            if not os.path.isdir(camera_path):
                print(f'Warning: camera {camera} is not present in {subject}/{action}')
                continue
            ...
            table_segment['camera_idx'] = camera_idx
            ...
            retval['table'].append(table_segment) # append segement per camera
    pbar.update(1)
pbar.close()
