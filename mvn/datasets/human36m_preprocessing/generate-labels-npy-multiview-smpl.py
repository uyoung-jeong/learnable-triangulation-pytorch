"""
    Generate 'labels.npy' for multiview 'human36m.py' with smpl keypoint setting
    from https://github.sec.samsung.net/RRU8-VIOLET/multi-view-net/

    Usage: `python3 generate-labels-npy-multiview-smpl.py <path/to/Human3.6M-root> <path/to/una-dinosauria-data/h36m> <path/to/bboxes-Human36M-squared.npy>`

python generate-labels-npy-multiview-smpl.py \
/home/uyoung/human_pose_estimation/datasets/Human36M \
/home/uyoung/human_pose_estimation/datasets/Human36M/extra/una-dinosauria-data/h36m \
/home/uyoung/human_pose_estimation/datasets/Human36M/extra/bboxes-Human36M-GT.npy
"""
import os, sys
import numpy as np
import h5py
...
table_dtype = np.dtype([
    ...
    ('keypoints_smpl', np.float32, (24,4)), # SMPL format
...
destination_file_path = os.path.join(h36m_root, "extra", f"human36m-multiview-smpl-labels-{BBOXES_SOURCE}bboxes.npy")
...
for subject_idx, subject in enumerate(retval['subject_names']):
    ...
    for action_idx, action in enumerate(retval['action_names']):
        ...
        # https://github.com/nkolot/SPIN/blob/master/datasets/preprocess/h36m_train.py
        ...
        with h5py.File(os.path.join(una_dinosauria_root, subject, 'MyPoses', '3D_positions',
            ...
            S17 = np.array(poses_file['3D_positions']).T.reshape(-1, 32, 3)[frame_idxs][:, h36m_idx]

        S24 = np.zeros((S17.shape[0], 24, 4))
        ...
        table_segment['keypoints_smpl'] = S24
