"""
    ...
    'Segments_mat_gt_pl_{subject}.tgz' files should be downloaded and extracted.
    To download and extract those files, 
    please use forked version of h36m-fetch repo below:
    https://github.com/uyoung-jeong/h36m-fetch
...
if len(subjects) < 1:
    ...
    exit()
print("subjects to process: {}".format(subjects))
...
for subj in subjects:
    ...
    for act in img_actions:
        ...
        for cam in cameras:
            cam_path = join(act_path, cam)
            os.makedirs(cam_path, exist_ok=True)


# multiprocessing routine
def routine(img_subj_path, cameras, subj, action, files, save_dir):
    ...
    for cam in cameras:
        ...
        with h5py.File(selected_f, 'r') as mat:
            pbar = tqdm(total=len(img_files), desc='{}/{}/{}'.format(subj, action, cam))
            for img_f in img_files:
                ...
                # save
                #cv2.imwrite(join(save_dir, subj, action, cam, '{}_raw.jpg'.format(img_index)), mask)
                #cv2.imwrite(join(save_dir, subj, action, cam, '{}_aug.jpg'.format(img_index)), augmented)
                cv2.imwrite(join(save_dir, subj, action, cam, 'att_{}.jpg'.format(str(img_index).zfill(6))), rgba)
                pbar.update(1)
            pbar.close()
...
#pbar = tqdm(total=len(subjects), desc='processing')
for subj in subjects:
    ...
    async_res = [pool.apply_async(
            routine,
            args=(img_subj_path, cameras, subj, action, files, save_dir)
